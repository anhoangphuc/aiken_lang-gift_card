import { Constr, Lucid, MintingPolicy, OutRef, SpendingValidator, applyDoubleCborEncoding, applyParamsToScript, fromText } from "https://deno.land/x/lucid@0.10.1/mod.ts";
...
export function readValidators(): Validators {
    ...
    if (!redeem) {
        throw new Error("Reddem validator not found");
    }

    const giftCard = blueprint.validators.find((v) => v.title === "oneshot.gift_card");
...
export function applyParams(
    ...
    const policyId = lucid.utils.validatorToScriptHash({ type: "PlutusV2", script: giftCard });
